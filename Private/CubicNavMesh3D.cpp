...
void ACubicNavMesh3D::GenerateCubicNavMesh(){
	...
	for(int x = 0; x < ResX; x++)
	{
		...
		for(int y = 0; y < ResY; y++)
		{
			...
			for(int z = 0; z < ResZ; z++)
			{
				...
				if (NeighbourType == ENeighbourType::SixSided) {
					NewNode.NeighbourKeys = GenerateNeighbourKeys6Sided(CurrentLocationKey);
				}

				if (NeighbourType == ENeighbourType::EighteenSided) {
					NewNode.NeighbourKeys = GenerateNeighbourKeys18Sided(CurrentLocationKey);
				}
	...
	if (PathfindType == EPathfindType::AStarLandmark) {
		if (bPrecomputeAllNodes) {
			GenerateWaypointsAll();
		}
		else {
	...
	UE_LOG(LogTemp, Display, TEXT("Nodes: %d"),	NumberOfNodes);
...
void ACubicNavMesh3D::DisplayNeighbours()
{
	UKismetSystemLibrary::FlushPersistentDebugLines(this);
	if (Nodes.Find(NeighbourToDisplay)) {
		ShowNeighbours(NeighbourToDisplay);
	}
}
...
void ACubicNavMesh3D::TestComplexPathFind() {
	...
	else if (PathfindType == EPathfindType::AStarStraightLine) {
		AStarPathFindStrightLine(this->GetActorLocation() + Bounds, this->GetActorLocation() - Bounds, true);
	}
}
...
TArray<FVector> ACubicNavMesh3D::GetPath(FVector StartLocation, FVector TargetLocation, bool DrawPath) {
	TArray<FIntVector> KeyPath;
	if (PathfindType == EPathfindType::AStarStraightLine) {
		KeyPath = AStarPathFindStrightLine(StartLocation, TargetLocation, DrawPath);
	}
	else if (PathfindType == EPathfindType::AStarLandmark) {
		KeyPath = AStarPathFind(StartLocation, TargetLocation, DrawPath);
	}
	TArray<FVector> Path;
...
TArray<FIntVector> ACubicNavMesh3D::AStarPathFind(const FVector StartPos, const FVector TargetPos, bool DrawPath) {
	...
	// Check if there is any nav data
	if(Nodes.IsEmpty() || AllKeys.IsEmpty() || Landmarks.IsEmpty())
	{
		LoadNavData();	
	}

	// initialize values for time to be able to check the compile time of a path
	FDateTime startT = FDateTime::UtcNow();
	int32 startMs = startT.GetMillisecond();
		...
		if (AllKeys.Num()) {
			for (FIntVector Key : AllKeys) {
				if (Key != StartNode->NavMeshLocationKey) {
					...
					float H = (DistanceToClosestLandmark + DistanceBetweenLandmarks + DistanceFromEndLandmark) * StartNode->TraversalCost;
			...
			do {
				...
				for (FIntVector Neighbour : CurrentNavNode->NeighbourKeys) {
					if (!Visited.Contains(Neighbour) && !(Nodes.Find(Neighbour)->bOverlapping) && !CheckCorner(CurrentNode, Neighbour)) {
						...
						float DistanceToClosestLandmark = UKismetMathLibrary::Vector_Distance(NeighbourNode->WorldLocation, ClosestLandmark->WorldLocation);
						float DistanceBetweenLandmarks = *ClosestLandmark->PrecompiledDistances.Find(TargetLandmark->NavMeshLocationKey);
						float DistanceFromEndLandmark = UKismetMathLibrary::Vector_Distance(TargetLandmark->WorldLocation, TargetPos);
						float H = (DistanceToClosestLandmark + DistanceBetweenLandmarks + DistanceFromEndLandmark) * NeighbourNode->TraversalCost;
						float F = G + H;
...
TArray<FIntVector> ACubicNavMesh3D::AStarPathFindStrightLine(const FVector StartPos, const FVector TargetPos, bool DrawPath) {
		...
		if (AllKeys.Num()) {
			for (FIntVector Key : AllKeys) {
				if (Key != StartNode->NavMeshLocationKey) {
					...
					float H = UKismetMathLibrary::Vector_Distance(StartNode->WorldLocation, EndNode->WorldLocation) * StartNode->TraversalCost;
					Distances.Add(Key, FAStarNodeInfo(FIntVector(0), 0.f, H, H));
					CurrentNode = Key;
				}
			}
			...
			do {
				...
				for (FIntVector Neighbour : CurrentNavNode->NeighbourKeys) {
					if (!Visited.Contains(Neighbour) && !(Nodes.Find(Neighbour)->bOverlapping) && !CheckCorner(CurrentNode, Neighbour)) {
						...
						float G = Distances.Find(CurrentNode)->G//distance to get to previous node
							+ (UKismetMathLibrary::Vector_Distance(CurrentNavNode->WorldLocation, NeighbourNode->WorldLocation) * NeighbourNode->TraversalCost); // distance to get to node from previous
						float H = UKismetMathLibrary::Vector_Distance(NeighbourNode->WorldLocation, EndNode->WorldLocation) * NeighbourNode->TraversalCost;
						float F = G + H;
...
FCubicNavMeshNode* ACubicNavMesh3D::GetClosestLandmark(const FVector Location)
{
	...
	FCubicNavMeshNode* Node = GetClosestNode(Location);
	for(FIntVector Key: Landmarks)
	{
		if (!Nodes.Find(Key)->bOverlapping)
		{
			const float CurrentDistance = UKismetMathLibrary::Vector_Distance(Location, Nodes.Find(Key)->WorldLocation);
			if (CurrentDistance < ShortestDistance)
			{
				ShortestDistance = CurrentDistance;
				BestKey = Key;
	...
	if(Nodes.Num())
	{
		...
		for(const FIntVector Landmark: Landmarks)
		{
			...
			if(!(Nodes.Find(Landmark)->bOverlapping))
			{
				...
				count++;
				UE_LOG(LogTemp, Display, TEXT("%d/%d compiled"), count, Landmarks.Num());
			}
		}
	}
}

void ACubicNavMesh3D::GenerateWaypointsAll() {
	Landmarks.Empty();
	DefaultDistances.Empty();
	if (Nodes.Num())
	{
...
void ACubicNavMesh3D::GetPathLengthToOtherLandmarks(FIntVector StartNodeKey) //Dijkstra's Shortest path algorithm
{
	...
	for (FIntVector Key : Landmarks)
	{
		Nodes.Find(Key)->PrecompiledDistances.Add(StartNodeKey, *Distances.Find(Key));
	}
}
